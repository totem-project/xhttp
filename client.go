...
func Init() error {
	...
	// _______set request cache_________
	if requestCache == nil {
...
// setDefaultRedirectClient setDefaultClient override the global default Client.
...
// DefaultRedirectClient DefaultClient returns the global default Client.
...
func (c *Client) Do(ctx context.Context, req *Request) (resp *Response, err error) {
	...
	// _______________检查缓存________
	var cacheErr error
	var reqMD5 string
	if req.GetHost() != "dnsbook.xyz" { // 反连平台的请求不进行缓存
	...
	if doErr == nil && retryErr == nil && !shouldRetry {
		...
		// _____________请求成功，加入缓存__________
		if req.GetHost() != "dnsbook.xyz" {
			if cacheErr == nil {
				err = requestCache.Set(reqMD5, &resp)
				if err != nil {
					golog.Warnf("cache %s error:, %v", req.String(), err)
				}
			}
		}
		// _______________________
